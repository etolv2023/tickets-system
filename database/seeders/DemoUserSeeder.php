<?php

namespace Database\Seeders;

use App\Enums\UserSkill;
use App\Models\Role;
use App\Models\User;
use Illuminate\Database\Seeder;

/**
 * Development only — never called by the installer. Covers every role plus the
 * three edge cases worth clicking through: a both-skills developer, a locked-out
 * user, and one forced to change their password (CLAUDE.md § 7.7).
 */
class DemoUserSeeder extends Seeder
{
    public const PASSWORD = 'etolv@2026';

    public function run(): void
    {
        $roleIds = Role::pluck('id', 'key');

        $users = [
            ['محمود كامل', 'admin@etolv.test', 'admin', UserSkill::None, []],
            ['هدى شعراوي', 'manager@etolv.test', 'manager', UserSkill::None, []],
            ['كريم عبد الله', 'support@etolv.test', 'support', UserSkill::None, []],
            ['نورهان سامي', 'frontend@etolv.test', 'frontend', UserSkill::Frontend, []],
            ['طارق الشناوي', 'backend@etolv.test', 'backend', UserSkill::Backend, []],
            // skills = both puts this one in BOTH assignment lists. F00.3
            ['ياسمين فؤاد', 'fullstack@etolv.test', 'frontend', UserSkill::Both, []],
            ['أحمد منصور', 'tester@etolv.test', 'tester', UserSkill::None, []],
            // is_active = false must be refused at login with a clear message. F00.2
            ['سلمى راضي', 'inactive@etolv.test', 'support', UserSkill::None, ['is_active' => false]],
            // Mimics an Excel-imported user: blocked until they set a password. F02
            ['عمر الديب', 'imported@etolv.test', 'backend', UserSkill::Backend, ['must_change_password' => true]],
        ];

        foreach ($users as [$name, $email, $roleKey, $skills, $extra]) {
            User::updateOrCreate(
                ['email' => $email],
                array_merge([
                    'name' => $name,
                    'password' => self::PASSWORD,
                    'role_id' => $roleIds[$roleKey],
                    'skills' => $skills,
                    'daily_capacity_hours' => 6.00,
                    'is_active' => true,
                    'must_change_password' => false,
                ], $extra)
            );
        }
    }
}
